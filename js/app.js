...
const selectedValue = document.getElementById('itemPrice');
const hiddenField = document.getElementById('hiddenPrice');
sizeRadios.forEach(radio => {
  radio.addEventListener('change', event => {
    selectedValue.textContent = `$ ${radio.value}`;
...
// Add to cart
// Get the submit button and radio button groups
const submitBtn = document.querySelector(".item-detail__para__addcart-submit");
const radioBtns1 = document.getElementsByName("type");
const radioBtns2 = document.getElementsByName("size");

// Attach an event listener to the submit button
submitBtn.addEventListener("click", function(event) {
  // Check if any option is not selected in the first radio button group
  let radioBtnGroup1Selected = false;
  for (let i = 0; i < radioBtns1.length; i++) {
    if (radioBtns1[i].checked) {
      radioBtnGroup1Selected = true;
      break;
    }
  }

  // Check if any option is not selected in the second radio button group
  let radioBtnGroup2Selected = false;
  for (let i = 0; i < radioBtns2.length; i++) {
    if (radioBtns2[i].checked) {
      radioBtnGroup2Selected = true;
      document.getElementById("hiddenPrice").value = radioBtns2[i].value;
      break;
    }
  }

  // If any option is not selected in either group, show an alert and prevent form submission
  if (!radioBtnGroup1Selected || !radioBtnGroup2Selected) {
    alert("Please select an option from both radio button groups.");
...
const input1 = document.getElementById('cart_qty1');
const input2 = document.getElementById('cart_qty2');
